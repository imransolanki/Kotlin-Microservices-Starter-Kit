...
class PetRepositoryImpl : PetRepository {
    ...
    override suspend fun create(pet: Pet): Long = newSuspendedTransaction {
        PetsEntity.new {
            name = pet.name
            status = pet.status
    ...
    override suspend fun update(pet: Pet): Boolean = newSuspendedTransaction {
    ...
    override suspend fun findById(petId: Long): Pet? = newSuspendedTransaction {
        PetsEntity.findById(petId)?.let {
            Pet(name = it.name, status = it.status, photoUrl = it.photoUrl, id = it.id.value)
        }
    }

    override suspend fun deleteByName(petName: String): Int = newSuspendedTransaction {
